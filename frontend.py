...
.pipeline-step:nth-child(3)::before { border-color: var(--accent4); }
...
/* ── Metrics ── */
.metrics-grid {
    display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin: 2rem 0;
}
.metric-card {
    background: linear-gradient(135deg, var(--card1) 0%, var(--card2) 100%);
...
.metric-card:nth-child(3)::before { background: var(--accent4); }
...
.metric-card:nth-child(3) .metric-value { color: var(--accent4); }
.metric-label {
...
<div style="position:relative; height:320px; border-radius:24px; overflow:hidden;">
  ...
  <div style="position:absolute; top:0; left:0; width:100%; height:320px;
    ...
    <div style="color:var(--text-mute); font-size:1rem; max-width:520px; line-height:1.6;
      ...
      Flight, hotel, and itinerary checks run together to build a practical travel plan
    </div>
    <div style="display:flex; gap:40px; margin-top:30px;">
      ...
      <div style="text-align:center;">
        <div style="font-family:'Playfair Display',serif; font-size:1.5rem; font-weight:700; color:var(--accent);">Itinerary</div>
        <div style="font-size:0.7rem; color:var(--text-faint); text-transform:uppercase; letter-spacing:0.1em; margin-top:2px;">Day-by-Day Plan</div>
      </div>
    </div>
  </div>
</div>
...
document.querySelectorAll('.sug-card').forEach(function(card) {
  ...
  card.addEventListener('click', function() {
    ...
    if (text && window.parent && window.parent.document) {
...
<div style="margin: 2.5rem 0 1rem;">
    ...
    <div style="font-family:'Playfair Display',serif; font-size:1.4rem; font-weight:700; color:var(--text-strong);">Popular Trip Ideas</div>
</div>
""", unsafe_allow_html=True)
...
<div class="input-container">
    <div style="display:inline-flex; align-items:center; gap:8px; margin-bottom:1rem;">
        ...
        <span style="font-size:0.7rem; font-weight:700; letter-spacing:0.2em; text-transform:uppercase; color:var(--accent);">
            Describe Your Trip
        </span>
    </div>
""", unsafe_allow_html=True)
    ...
    if not user_query.strip():
        ...
        if collected["final_response"]:
            ...
            <div style="margin: 2.5rem 0 1rem;">
                ...
                <div style="font-family:'Playfair Display',serif; font-size:1.4rem; font-weight:700; color:var(--text-strong);">Your Complete Travel Plan</div>
            </div>
            """, unsafe_allow_html=True)
            st.markdown(f'<div class="final-plan-card"><div class="final-plan-content">{collected["final_response"]}</div></div>',
                        unsafe_allow_html=True)
...
"""
        ...
        <div class="action-bar">
            <div class="action-bar-text">📁 Auto-saved → <code>travel_plans/{filename}</code></div>
        </div>
        """.format(filename=filename), unsafe_allow_html=True)
